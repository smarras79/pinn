...
epochs = 6000 # 6000
epochsLeftOfHydraulicJump = 35000 # 36000
epochsRightOfHydraulicJump = 28000 #8000
learning_rate = 1e-3
num_time_steps = 20
#Scheduler tuning parameters
scheduler_step_size_frequency = 10 #Number of times we want scheduler to reduce LR during full training with epochs
...
x_bump_left = 8.0
x_bump_center = 10.0
x_bump_center_with_offset = 11.5
x_bump_right = 12.0
bump_height = 0.2
bump_width = 4
...
lossLeftOfHydraulicJump_history = []
lossRightOfHydraulicJump_history = []
...
class ImprovedPINN_SWE(nn.Module):
    ...
    def forward(self, x):
        ...
        u_raw = q_val/torch.clamp(h_raw, min=epsilon)
        return torch.clamp(h_raw, min=epsilon),torch.clamp(u_raw, min=epsilon)
...
optimizerLeft = optim.Adam(model.parameters(), lr=learning_rate)
schedulerLeft = optim.lr_scheduler.StepLR(optimizerLeft, epochsLeftOfHydraulicJump // scheduler_step_size_frequency, scheduler_gamma)

optimizerRight = optim.Adam(model.parameters(), lr=learning_rate)
schedulerRight = optim.lr_scheduler.StepLR(optimizerRight, epochsRightOfHydraulicJump // scheduler_step_size_frequency, scheduler_gamma)
...
def improved_physics_loss(h, u, x, epoch,epochs):
    ...
    # total_pde_loss = lambda_c * continuity_loss + lambda_m * momentum_loss 
...
def train():
    ...
    for epoch in range(epochs):
        ...
        #u_collocation = q_val/h_collocation
        ...
        #u_boundary_left = q_val/h_boundary_left

        h_boundary_right,u_boundary_right  = model(x_boundary_right)
        # u_boundary_right = q_val/h_boundary_right
        ...
        if (epoch + 1) % 100 == 0:
            ...
            print(f"  Froude number max: {torch.max(Fr):.4e}")
            print(f"  Froude number min: {torch.min(Fr):.4e}")
            print(f"  Learning Rate: {optimizer.param_groups[0]['lr']:.2e}")
    #End of Training loop

    #2) Hydraulic jump detection and x range of supercritical area
    fr_critical_mask = (Fr > 1.0).float()
    x_collocation_fr_critical = fr_critical_mask * x_collocation
    x_collocation_fr_critical = x_collocation_fr_critical[x_collocation_fr_critical>1]
    x_hydraulic_jump_left = torch.min(x_collocation_fr_critical)
    x_hydraulic_jump_right = torch.max(x_collocation_fr_critical)
    print(f"x_hydraulic_jump_left: {x_hydraulic_jump_left}")
    print(f"x_hydraulic_jump_right: {x_hydraulic_jump_right}")

    #3) Training in x range (x_hydraulic_jump_left,x_bump_center) with indication to NN have a jump in height
    print("Refinement training loop in left of hydraulic jump region")
    x_hydraulic_jump_left_collocation = torch.cat([torch.linspace(x_hydraulic_jump_left, x_bump_center_with_offset, num_collocation_points)]).reshape(-1, 1)
    x_hydraulic_jump_left_collocation.requires_grad_(True)

    for epoch in range(epochsLeftOfHydraulicJump):
        optimizerLeft.zero_grad()
        # Boundary loss
        h_boundary_left,u_boundary_left = model(x_boundary_left)
        ...
        h_around_collocation_bump,u_around_collocation_bump = model(x_hydraulic_jump_left_collocation)
        loss_pde, pde_components = improved_physics_loss(h_around_collocation_bump, u_around_collocation_bump, 
                                                        x_hydraulic_jump_left_collocation,epoch,epochsLeftOfHydraulicJump)
        ...
        #loss_constraint = model.constraint_loss(x_hydraulic_jump_left_collocation,h_around_collocation_bump)
        loss_constraint = model.constraint_height_loss_before_bump(x_hydraulic_jump_left_collocation,h_around_collocation_bump)

        # Hydraulic jump detection and height constraint
        epsilon = 1e-3
        Fr = u_around_collocation_bump / torch.sqrt(g * torch.clamp(h_around_collocation_bump, min=epsilon))

        lossLeftOfHydraulicJump = (
            loss_pde \
            + loss_boundary \
            + 1.1 * loss_constraint \
        )
        
        lossLeftOfHydraulicJump.backward()
        
        # Gradient clipping
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        optimizerLeft.step()
        schedulerLeft.step()
        lossLeftOfHydraulicJump_history.append(lossLeftOfHydraulicJump.item())
        ...
        if (epoch + 1) % 100 == 0:
            print(f"Epoch {epoch+1}/{epochsLeftOfHydraulicJump}")
            print(f"  Total Loss: {lossLeftOfHydraulicJump.item():.4e}")
            print(f"  Boundary Loss: {loss_boundary.item():.4e}")
            print(f"  PDE Loss: {loss_pde.item():.4e}")
            print(f"  Continuity: {pde_components['continuity']:.4e}")
            print(f"  Momentum: {pde_components['momentum']:.4e}")
            ...
            print(f"  Froude number max: {torch.max(Fr):.4e}")
            print(f"  Learning Rate: {optimizerLeft.param_groups[0]['lr']:.2e}")
    print("End of Refinement training loop in left of hydraulic jump region")
    #End of Training loop - LeftOfHydraulicJump

    #4) Training in x range (x_bump_center,x_max)
    print("Refinement training loop in right of hydraulic jump region")
    x_hydraulic_jump_right_collocation = torch.cat([torch.linspace(x_bump_center_with_offset,x_max, num_collocation_points)]).reshape(-1, 1)
    x_hydraulic_jump_right_collocation.requires_grad_(True)

    for epoch in range(epochsRightOfHydraulicJump):
        optimizerRight.zero_grad()
        # Boundary loss
        h_boundary_left,u_boundary_left = model(x_boundary_left)
        ...
        h_around_collocation_bump,u_around_collocation_bump = model(x_hydraulic_jump_right_collocation)
        loss_pde, pde_components = improved_physics_loss(h_around_collocation_bump, u_around_collocation_bump, 
                                                        x_hydraulic_jump_right_collocation,epoch,epochsRightOfHydraulicJump)

        
        # Height contraint loss in "bump region". Height should not penetrate the bump
        loss_constraint = model.constraint_height_loss_after_hydraulic_jump(h_around_collocation_bump)
        ...
        lossRightOfHydraulicJump = (
            loss_pde \
            + loss_boundary \
            + 0.9 * loss_constraint \
        )
        
        lossRightOfHydraulicJump.backward()
        
        # Gradient clipping
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        optimizerRight.step()
        schedulerRight.step()
        lossRightOfHydraulicJump_history.append(lossRightOfHydraulicJump.item())
        ...
        if (epoch + 1) % 100 == 0:
            print(f"Epoch {epoch+1}/{epochsRightOfHydraulicJump}")
            print(f"  Total Loss: {lossRightOfHydraulicJump.item():.4e}")
            print(f"  Boundary Loss: {loss_boundary.item():.4e}")
            print(f"  PDE Loss: {loss_pde.item():.4e}")
            print(f"  Continuity: {pde_components['continuity']:.4e}")
            print(f"  Momentum: {pde_components['momentum']:.4e}")
            # print(f"  Bump Height penetration Loss: {loss_constraint.item():.4e}")
            print(f"  Froude number max: {torch.max(Fr):.4e}")
            print(f"  Learning Rate: {optimizerRight.param_groups[0]['lr']:.2e}")
    print("End of Refinement training loop in left of hydraulic jump region")
    #End of Training loop - RightOfHydraulicJump
    ...
    for epoch in range(epochsRefinement):
        ...
        #u_around_collocation_bump = q_val/h_around_collocation_bump
        loss_pde, pde_components = improved_physics_loss(h_around_collocation_bump, u_around_collocation_bump, 
...
def test(loss,start_time):
    ...
    with torch.no_grad():
        ...
        # Create plots
        plot(x_np,h_pred_plot,u_pred_plot,zb_plot,h_exact,Fr_plot,fr_critical_mask_plot,h_error,h_error_ratio,elapsed_time,loss)

        plotSizeUp(x_np,h_pred_plot,u_pred_plot,zb_plot,h_exact,Fr_plot,fr_critical_mask_plot,h_error,h_error_ratio,elapsed_time,loss)
    ...
    # Loss LeftOfHydraulicJump history plot
    plt.figure(figsize=(10, 6))
    plt.semilogy(lossLeftOfHydraulicJump_history, 'b-', linewidth=2)
    plt.xlabel('Epoch')
    plt.ylabel('Total Loss')
    plt.title('Training Loss History(Super)')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dir, 'lossLeftOfHydraulicJump_history.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def plot(x_np,h_pred_plot,u_pred_plot,zb_plot,h_exact,Fr_plot,fr_critical_mask_plot,h_error,h_error_ratio,elapsed_time,loss):
        # Create plots
        ...
        # Water height
        ax1.plot(x_np, h_pred_plot, 'b-', label='PINN h(x,t)', linewidth=lw1)
        ax1.plot(x_np, h_exact, 'r--', label='Analytical solution', linewidth=lw1, alpha=0.8)
        ax1.plot(x_np, zb_plot, 'g--', label='Bottom topography zb(x)', linewidth=lw2)
        ax1.set_xlabel('x [m]', fontname="Arial", fontsize=legendFsize, weight='bold')
        ax1.set_ylabel('Water Height h(x) [m]', fontname="Arial", fontsize=legendFsize, weight='bold')
        ...
        # Velocity
        ax2.plot(x_np, u_pred_plot, 'b-', label='PINN u(x,t)', linewidth=lw1)
        ax2.plot(x_np, zb_plot, 'g--', label='Bottom topography zb(x)', linewidth=lw2)
        ax2.set_xlabel('x [m]', fontname="Arial", fontsize=legendFsize, weight='bold')
        ax2.set_ylabel('Velocity u(x) [m/s]', fontname="Arial", fontsize=legendFsize, weight='bold')
        ...
        #Froude number plot
        ax3.plot(x_np, Fr_plot, 'b-', label='Froude number', linewidth=lw1)
        ax3.plot(x_np, fr_critical_mask_plot, 'r-', label='Froude number mask', linewidth=lw1)
        ax3.plot(x_np, zb_plot, 'g--', label='Bottom topography zb(x)', linewidth=lw2)
        ax3.set_xlabel('x [m]', fontname="Arial", fontsize=legendFsize, weight='bold')
        ax3.set_ylabel('Froude number', fontname="Arial", fontsize=legendFsize, weight='bold')
        ...
        # Plot height error
        ax4.plot(x_np, h_error, 'r-', label='h_pred - h_exact', linewidth=lw1)
        ax4.plot(x_np, h_error_ratio, 'b-', label = '(h_pred - h_exact)/h_exact', linewidth=lw1)
        ax4.set_xlabel('x [m]', fontname="Arial", fontsize=legendFsize, weight='bold')
        ax4.set_ylabel('|h_pred - h_exact|', fontname="Arial", fontsize=legendFsize, weight='bold')
        ...
        plt.figtext(0.5, 0.0, info_text, ha='center', fontsize=legendFsize)
        plt.savefig(os.path.join(output_dir, f"swe_solution.png"), dpi=300, bbox_inches='tight')
        plt.close()


def plotSizeUp(x_np,h_pred_plot,u_pred_plot,zb_plot,h_exact,Fr_plot,fr_critical_mask_plot,h_error,h_error_ratio,elapsed_time,loss):
        # Create plots
        ...
        ax1.plot(x_np, h_exact, 'r--', label='Analytical solution', linewidth=lw1, alpha=0.8)
        ax1.plot(x_np, zb_plot, 'g--', label='Bottom topography zb(x)', linewidth=lw2)
        ax1.set_xlabel('x [m]', fontname="Arial", fontsize=fsize , weight='bold')
        ax1.set_ylabel('Water Height h(x) [m]', fontname="Arial", fontsize=fsize, weight='bold')
        ax1.set_title(f'Water Height')
        ax1.legend(fontsize=legendFsize)
        ...
        ax2.plot(x_np, u_pred_plot, 'b-', label='PINN u(x,t)', linewidth=lw1)
        ax2.plot(x_np, zb_plot, 'g--', label='Bottom topography zb(x)', linewidth=lw2)
        ax2.set_xlabel('x [m]', fontname="Arial", fontsize=fsize, weight='bold')
        ax2.set_ylabel('Velocity u(x) [m/s]', fontname="Arial", fontsize=fsize , weight='bold')
        ax2.set_title(f'Velocity')
        ax2.legend(fontsize=legendFsize)
        ...
        ax3.plot(x_np, fr_critical_mask_plot, 'r-', label='Froude number mask', linewidth=lw1)
        ax3.plot(x_np, zb_plot, 'g--', label='Bottom topography zb(x)', linewidth=lw2)
        ax3.set_xlabel('x [m]', fontname="Arial", fontsize=fsize , weight='bold')
        ax3.set_ylabel('Froude number', fontname="Arial", fontsize=fsize , weight='bold')
        ax3.set_title(f'Froude number')
        ax3.legend(fontsize=legendFsize)
        ...
        ax4.plot(x_np, h_error, 'r-', label='h_pred - h_exact', linewidth=lw1)
        ax4.plot(x_np, h_error_ratio, 'b-', label = '(h_pred - h_exact)/h_exact', linewidth=lw1)
        ax4.set_xlabel('x [m]', fontname="Arial", fontsize=fsize , weight='bold')
        ax4.set_ylabel('|h_pred - h_exact|', fontname="Arial", fontsize=fsize , weight='bold')
        ax4.set_title(f'Height Error - Max: {np.max(h_error):.4f}')
        ax4.legend(fontsize=legendFsize)
        ...
        info_text = f"Epochs: {epochs} | Points: {num_collocation_points}\n"
        info_text += f"Training time: {elapsed_time:.1f}s | Final loss: {loss.item():.4e}"
        plt.figtext(0.5, 0.0, info_text, ha='center', fontsize=fsize)
        plt.savefig(os.path.join(output_dir, f"swe_solution_sizeup.png"), dpi=300, bbox_inches='tight')
        plt.close()
