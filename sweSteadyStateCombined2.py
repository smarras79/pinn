...
#Bump left region supercritical
x_before_bump_left = 0
x_before_bump_right = 8

#Bump Region subcritical
x_bump_left_sub = 28.0
x_bump_center_sub = 30.0
x_bump_right_sub = 32.0
bump_height_sub = 0.04
bump_width_sub = 4

#Bump left region subcritical
x_before_bump_left_sub = 20
x_before_bump_right_sub = 28


gradient_based_weighting = True

# Output directory
output_dir = "swe/temp/swe_steady_combined_" + str(epochs)
...
def bed_elevation(x: torch.Tensor) -> torch.Tensor:
    ...
    zb_h_sub = bump_height_sub - (bump_height_sub/bump_width_sub) * (x - x_bump_center_sub) **2
    return torch.where((x > x_bump_left) & (x < x_bump_right), zb_h, 
                       torch.where((x > x_bump_left_sub) & (x < x_bump_right_sub), zb_h_sub,zb))
...
def boundary_condition_loss_right_side(h_right_pred, u_right_pred):
...
def train():
    ...
    for epoch in range(epochs):
        ...
        pde_weight = 1
        bc_weight = 1
        loss_constraint_weight = 1.5 #In Bump: Height should not penetrate the bump
        loss_constraint_before_bump_weight = 1 #Before bump: Height should not be below eta_val
        loss_constraint_velocity_weight = 3 #Full domain: Velocity should not get negative
        loss = (
            pde_weight * loss_pde
    ...
    for epoch in range(epochsRefinement):
        ...
        # h_boundary_left, u_boundary_left = model(x_boundary_left)
        h_boundary_right, u_boundary_right = model(x_boundary_right)
        loss_boundary = boundary_condition_loss_right_side(h_boundary_right, u_boundary_right)
    ...
    # Training loop - Subcritical region
    print("Refinement training loop in subcritical region")
    # Subcritical region points
    x_bump_collocation_sub = torch.cat([torch.linspace(x_min_sub, x_max, num_collocation_points)]).reshape(-1, 1)
    x_bump_collocation_sub.requires_grad_(True)

    # Boundary points
    x_boundary_left_sub = torch.ones(num_boundary_points, 1) * x_min_sub
    x_boundary_right_sub = torch.ones(num_boundary_points, 1) * x_max
    x_boundary_left_sub.requires_grad_(True)
    x_boundary_right_sub.requires_grad_(True)

    # Bump region surrounding points
    x_around_bump_collocation_sub = torch.cat([torch.linspace(x_bump_left_sub-2, x_bump_right_sub+2, num_collocation_points)]).reshape(-1, 1)
    x_around_bump_collocation_sub.requires_grad_(True)
    epochsRefinement = 4000
    for epoch in range(epochsRefinement):
        optimizer.zero_grad()
        # Boundary loss
        h_boundary_left_sub, u_boundary_left_sub = model(x_boundary_left_sub)
        h_boundary_right_sub, u_boundary_right_sub = model(x_boundary_right_sub)
        loss_boundary_sub = boundary_condition_loss_both_side(h_boundary_left_sub, u_boundary_left_sub, 
                                            h_boundary_right_sub, u_boundary_right_sub)

        h_collocation_bump_sub, u_collocation_bump_sub = model(x_bump_collocation_sub)
        loss_pde_sub, pde_components_sub = improved_physics_loss(h_collocation_bump_sub, u_collocation_bump_sub, 
                                                        x_bump_collocation_sub,epoch,epochsRefinement)

        # Height contraint loss in "bump region". Height should not penetrate the bump
        h_around_collocation_bump_sub, u_around_collocation_bump_sub = model(x_around_bump_collocation_sub)
        loss_constraint_sub = model.constraint_loss(x_around_bump_collocation_sub,h_around_collocation_bump_sub)

        # Hydraulic jump detection and height constraint
        epsilon = 1e-3
        Fr = u_collocation_bump_sub / torch.sqrt(g * torch.clamp(h_collocation_bump_sub, min=epsilon))

        lossrefinement_sub = (
            loss_pde_sub \
            + loss_boundary_sub \
            + 2 * loss_constraint_sub \
        )
        
        lossrefinement_sub.backward()
        ...
        if (epoch + 1) % 100 == 0:
            print(f"Epoch {epoch+1}/{epochsRefinement}")
            print(f"  Total Loss: {lossrefinement_sub.item():.4e}")
            print(f"  Boundary Loss: {loss_boundary_sub.item():.4e}")
            print(f"  PDE Loss: {loss_pde_sub.item():.4e}")
            print(f"  Continuity: {pde_components_sub['continuity']:.4e}")
            print(f"  Momentum: {pde_components_sub['momentum']:.4e}")
            print(f"  Bump Height penetration Loss: {loss_constraint_sub.item():.4e}")
            print(f"  Froude number: {torch.max(Fr):.4e}")
    #End of Training loop - Subcritical region
