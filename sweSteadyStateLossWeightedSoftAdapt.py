...
import os
import time
from softadapt import SoftAdapt, LossWeightedSoftAdapt # Can choose different variants
...
scheduler_gamma=0.75 #Factor by which scheduler will reduce LR at each epoch interval
...
#Initialize the SoftAdapt object
# beta parameter controls the sensitivity to the change rate of the losses
softadapt_manager = LossWeightedSoftAdapt(beta=0.1,accuracy_order=6) 
...
class ImprovedPINN_SWE(nn.Module):
    ...
    def __init__(self):
        ...
        # Input layer
        self.input_layer = nn.Linear(1, 409)  

        # Hidden layers
        self.hidden_layers = nn.ModuleList()  
        self.hidden_layers.append(nn.Linear(409, 283))  
        self.hidden_layers.append(nn.Linear(283, 149))    
        self.hidden_layers.append(nn.Linear(149, 73))
        self.hidden_layers.append(nn.Linear(73, 37))  
        self.hidden_layers.append(nn.Linear(37, 13))    
        self.hidden_layers.append(nn.Linear(13, 7))

        # Output layers for the two results
        self.output_h = nn.Linear(7, 1)
        self.output_u = nn.Linear(7, 1)
...
def train():
    ...
    # Optional: Initialize manual weights as a starting point (SoftAdapt will adjust them)
    ...
    for epoch in range(epochs):
        ...
        pde_loss_history.append(loss_pde.clone().detach().numpy())
        bc_loss_history.append(loss_boundary.clone().detach().numpy())

        if len(pde_loss_history) > 100:
            pde_loss_history.pop(0)
            bc_loss_history.pop(0)

        # After a certain number of epochs, start using adaptive weights
        if epoch >= 100 and (epoch + 1) % 100 == 0: 
            # Calculate new weights based on loss history
            # The weights returned are a tensor, e.g., [w_pde, w_bc, w_ic]
            weights = softadapt_manager.get_component_weights(pde_loss_history, 
                                                              bc_loss_history)

        # Total loss
        pde_weight_val = weights.numpy()[0]
        bc_weight_val = weights.numpy()[1]
        loss = (
            pde_weight_val * loss_pde
            + bc_weight_val * loss_boundary
        )
        ...
        # bc_loss_history.append(loss_boundary.detach().item())
        ...
        if (epoch + 1) % 100 == 0:
            ...
            print(f"  pde_weight_val: {pde_weight_val.item():.4e}")
            print(f"  bc_weight_val: {bc_weight_val.item():.4e}")
