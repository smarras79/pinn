...
def zb_tensor_for_plotting(x_tensor):
    ...
    zb_np = np.where(
        ...
        bump_height - 0.5 * (x_np - bump_center)**2,
        np.where(((x_np > 28) & (x_np < 29)) | ((x_np > 31) & (x_np < 32)),
        bump_height - 0.5 * (x_np - bump_center)**2,
        0.0))
    return zb_np
...
def zb_tensor(x):
    ...
    return torch.where(
        ...
        bump_height - 0.5 * (x - bump_center)**2,
        torch.where(((x > 28) & (x < 29)) | ((x > 31) & (x < 32)),
        bump_height - 0.5 * (x - bump_center)**2,
        0.0))
    # return torch.where(
    #     (x > bump_center - bump_width / 2) & (x < bump_center + bump_width / 2),
...
x_fan = torch.rand(int(0.6 * num_collocation_points), 1) * (fan_right - fan_left) + fan_left
x_outer = torch.rand(int(0.4 * num_collocation_points), 1) * (x_max - x_min) + x_min
